...
生成数据：
    1. Project「e2e-demo-project」+ TestRun（已完成）
    2. 10 条 TestCase
...
async def _get_or_create_test_run(db, project_id: uuid.UUID, user_id: uuid.UUID) -> TestRun:
    """幂等：按 project+name 查找 test_run。"""
    row = (
        await db.execute(
            select(TestRun).where(
                TestRun.project_id == project_id,
                TestRun.source_ref == TEST_RUN_NAME,
            )
        )
    ).scalar_one_or_none()
    if row:
...
async def main() -> None:
    ...
    async with AsyncSessionLocal() as db:
        ...
        logger.info("=" * 60)
        logger.info("下一步：进「知识库RAG」页 → 重建 → 应有非 0 切片数")
