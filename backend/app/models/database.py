...
class ApiEndpoint(Base):
    ...
    is_active = Column(Boolean, default=True)
    source = Column(CasePairEnum(EndpointSource, values_callable=lambda x: [e.value for e in x], name="endpointsource"), default=EndpointSource.DOC_IMPORT, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Scenario(Base):
    ...
    status = Column(CasePairEnum(ScenarioStatus, values_callable=lambda x: [e.value for e in x], name="scenariostatus"), default=ScenarioStatus.DRAFT, nullable=False)
